...
function puntua($mail){
  global $db;	
  
	$sql="UPDATE usuaris SET puntuacion_auto=puntuacion_auto+10 WHERE email='$mail'";
	//echo $sql;
...
function setRanking($nombre,$puntuacion){
   global $db;
	
	if($nombre != "" && $puntuacion != ""){
		
		$sql="INSERT INTO ranking(nombre,puntuacion,fecha) VALUES('$nombre','$puntuacion',NOW())";
		$res=$db->sql_query($sql);
		
		if ($res) return "<response valid='true'><mensaje>¡Puntuación guardada!</mensaje></response>"; 
		else return "<response valid='false'><mensaje>Error al guardar Puntuación. Por favor, inténtelo más tarde.</mensaje></response>"; 
	}else return "<response valid='false'><mensaje>Datos Incorrectos.</mensaje></response>"; 
...
function getRanking($num){
   global $db;
	...
	$sql = "SELECT * FROM ranking ORDER BY puntuacion ASC LIMIT 0,$num";
	$result = $db->sql_query($sql,true);
	$num_rows = $db->sql_numrows($result);
	
	if($num_rows != 0){
			$ret = "<response valid='true'>\n";
			$i=0;
		 	while($res = $db->sql_fetchrow($result)){
		 		$nombre =$res['nombre'];
				$puntuacion =$res['puntuacion'];
				$ret .= "<p$i nombre='$nombre' puntuacion='$puntuacion' />\n";
				$i++;
			}
			$ret .= "</response>";
			return $ret; 
