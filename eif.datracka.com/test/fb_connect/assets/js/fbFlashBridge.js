...
function FBFlashBridgeGetPhotosList(userId)
{
	api.photos_getAlbums([userId > 0 ? userId : api._session.uid], null, function(result, ex) 
	{					
		photoResult = result;
		
		trace("PHOTOS_LIST");
		
		if(!$.isArray(photoResult))
			photoResult = [];
		
		FBFlashBridgeDispatcher("PHOTOS_LIST");
		
		FBFlashBridgeFlashDispatcher("onPhotosList", photoResult);
...
function FBFlashBridgeGetPhotosImage(arrPhotos)
{
	api.photos_get(null, null, arrPhotos, function(result, ex) 
	{					
		photoImage = result;
		
		if(!$.isArray(photoImage))
			photoImage = [];
		
		FBFlashBridgeDispatcher("PHOTOS_IMAGE");
		FBFlashBridgeFlashDispatcher("onPhotosImage", photoImage);
	});
}
